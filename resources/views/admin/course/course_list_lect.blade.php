        ...
        <div>
            ...
            <table class="layui-table" align="center">
                ...
                @foreach ($data as $k=>$v)
                    ...
                    <td>{{$v->course_name}}</td>
                    <td>{{$v->cate_name}}</td>
                    <td><img src="/{{$v->course_page}}" width="60" height="60" /></td>
                    ...
                    <td><a href="/admin/course_update?course_id={{$v->course_id}}" class='btn layui-btn layui-btn-sm'>编辑</a>&nbsp;<a href='/admin/course_del?course_id={{$v->course_id}}' class='layui-btn layui-btn-sm'>删除</a><a href="/admin/catelog/index?course_id={{$v->course_id}}" class='btn layui-btn layui-btn-sm'>添加文章</a><a href="/admin/catelog/list?id={{$v->course_id}}" class='btn layui-btn layui-btn-sm'>文章列表</a>
                        <a href="/admin/question_list?id={{$v->course_id}}" class='btn layui-btn layui-btn-sm'>问答列表</a><a href="/admin/lect_order?course_id={{$v->course_id}}&lect_id={{$v->lect_id}}" class='layui-btn layui-btn-sm'>收益详情</a></td>
                    </td>
                    </tbody>
                @endforeach
            </table>
