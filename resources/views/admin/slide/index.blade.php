...
@section('content')
    <a href="{{ route('admin.slide.create') }}" class="layui-btn">添加轮播图</a>
        ...
        @foreach($slide as $s)
            <tr align="center">
                ...
                <td style="text-align: center;">
                    <a href="{{ route('admin.slide.edit') }}?slide_id={{ $s['slide_id'] }}"
    ...
    <script>
        ...
        function del(slide_id) {
            layer.confirm('你确定要删除这个轮播图吗？', {
                ...
                $.post("{{ route('admin.slide.delete') }}", {slide_id: slide_id},
                    function (data) {
                        if (data.code === 0) {
                            layer.msg('操作成功', {
                                offset: '15px'
                                ...
                                location.href = '{{ route('admin.slide.index') }}';
                            });
                        } else {
                            layer.msg(data.msg, {
                                offset: '15px'
